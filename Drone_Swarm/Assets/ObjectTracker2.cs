...
public class ObjectTracker2 : MonoBehaviour
{
    ...
    struct ObjData                          // storing data returned from raycast in struct
    {
        public bool Locked;
    ...
    void Update()
    {
        ...
        for(int i = 0; i < NumDetected; i++)    // starting at beginning of detected objects array, searches for all the detected objects
        {
            ...
            Vector3 VecTowObj = DetectedObjPos[i] - gameObject.transform.position;                  // vector towards object transform
            ...
            if (Physics.Raycast(transform.position, RaycastDir, out hit) && (NumSavedObj < MaxSavedObj))   // if detects the object, and there is room in SavedObjects array
            {
